<script lang="ts">
  ...
  function getScoreMessage(pct: number): string {
    ...
    if (pct >= 50) return 'Đạt! Bạn nên ôn tập lại thêm để đạt điểm số cao hơn.';
    return 'Chưa đạt! Bạn cần đọc kỹ lại tài liệu và làm lại bài thi ôn tập.';
  }

  // Start practicing
  function startTest() {
    const selectedSecIds = Object.keys(selectedSections).filter(id => selectedSections[id]);
    const selectedSecs = sections.filter(sec => selectedSecIds.includes(sec.id));
  ...
  // Option class mapping for review
...
<header>
  <div class="header-container">
    ...
    <p style="font-size: 0.9rem; color: var(--text-muted); margin-top: 0.25rem;">
      Phần mềm luyện tập trắc nghiệm kiến thức nghiệp vụ cơ sở
    </p>
  </div>
</header>
...
<main>
  {#if appState === 'setup'}
    ...
    <div class="card setup-card" style="max-width: 800px; margin: 0 auto;">
      <h2 style="margin-bottom: 1.5rem; color: var(--primary); font-family: var(--font-title); text-align: center;">
        CẤU HÌNH ĐỀ ÔN TẬP
      </h2>
      ...
      <div style="margin-bottom: 2rem;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
          <h3 style="font-size: 1.1rem; font-weight: 600;">Chọn chuyên đề học tập:</h3>
          <div style="display: flex; gap: 0.5rem;">
            <fluent-button appearance="accent" onclick={() => toggleAllSections(true)}>
              Chọn tất cả
            </fluent-button>
            ...
            <div class="topic-item">
              <label class="topic-label">
                <input 
                  ...
                  onchange={(e) => selectedSections[sec.id] = (e.target as HTMLInputElement).checked}
                />
                <span class="topic-text">
                  <strong>Chuyên đề {sec.id}:</strong> {sec.title} 
                  <span class="question-count">({sec.questions.length} câu)</span>
                </span>
              </label>
            </div>
          {/each}
      ...
      <div style="margin-bottom: 2.5rem;">
        <h3 style="font-size: 1.1rem; font-weight: 600; margin-bottom: 1rem;">Số lượng câu hỏi ôn tập:</h3>
        <div class="limit-selector">
          <label class="limit-option">
            <input type="radio" name="limit" value={0} checked={questionLimit === 0} onchange={() => questionLimit = 0} />
            <span>Tất cả các câu</span>
          </label>
          <label class="limit-option">
            <input type="radio" name="limit" value={10} checked={questionLimit === 10} onchange={() => questionLimit = 10} />
            <span>10 câu</span>
          </label>
          <label class="limit-option">
            <input type="radio" name="limit" value={20} checked={questionLimit === 20} onchange={() => questionLimit = 20} />
            <span>20 câu</span>
          </label>
          <label class="limit-option">
            <input type="radio" name="limit" value={30} checked={questionLimit === 30} onchange={() => questionLimit = 30} />
            <span>30 câu</span>
          </label>
          <label class="limit-option">
            <input type="radio" name="limit" value={50} checked={questionLimit === 50} onchange={() => questionLimit = 50} />
            <span>50 câu</span>
          </label>
        </div>
      </div>
  ...
  {:else}
    ...
    <div class="test-layout">
      ...
      <div class="questions-column">
        ...
        {#if appState === 'review'}
          <!-- Score Summary Card -->
          <div class="card result-card" style="border-left: 6px solid var(--primary);">
            <h2 style="font-family: var(--font-title); margin-bottom: 0.5rem;">KẾT QUẢ ÔN TẬP</h2>
            <div class="score-display">
              ...
              <span class="score-badge" class:badge-pass={scorePercentage >= 50}>{scorePercentage}%</span>
            </div>
        ...
        <!-- Questions List -->
        {#each sessionQuestions as q, qIdx}
          <div 
            id="question-card-{qIdx}" 
            class="card question-card" 
            class:active-card={qIdx === activeQuestionIndex}
          >
            <!-- Card Header -->
            <div class="q-header">
            ...
            {#if appState === 'practicing'}
              <fluent-radio-group 
                ...
                onchange={(e: Event) => {
                  ...
                  handleAnswerSelect(qIdx, val);
                }}
              >
                {#each q.shuffledOptions as opt, optIdx}
                  <fluent-radio value={opt.id.toString()}>
            ...
            {:else}
              ...
              <div style="display: flex; flex-direction: column; gap: 0.5rem; width: 100%;">
                {#each q.shuffledOptions as opt, optIdx}
                  ...
                  <div class="option-wrapper {optionClass}">
                    ...
                    {#if optionClass === 'correct'}
                      <span class="feedback-icon correct-icon" title="Đáp án đúng">✓ Đúng</span>
                    {:else if optionClass === 'wrong'}
                      <span class="feedback-icon wrong-icon" title="Lựa chọn của bạn (Sai)">✗ Lựa chọn sai</span>
                    {/if}
                  </div>
                {/each}
                ...
                <!-- Unanswered Warning -->
                {#if q.userAnswer === null}
                  <p style="color: var(--wrong-text); font-size: 0.9rem; font-weight: 500; margin-top: 0.5rem;">
        ...
        <div style="display: flex; justify-content: space-between; margin-top: 2rem;">
          <fluent-button onclick={resetTest}>
            Hủy / Quay lại
          </fluent-button>
          ...
          {#if appState === 'practicing'}
            <fluent-button 
              appearance="accent" 
              onclick={submitTest}
              style="--accent-fill-rest: var(--accent); --accent-fill-hover: var(--accent-light);"
            >
              Nộp bài thi ôn tập
            </fluent-button>
          {:else}
      ...
      <aside class="sidebar-column">
        <div class="card map-card">
          <h3 style="font-size: 1.1rem; font-weight: 700; margin-bottom: 0.5rem; font-family: var(--font-title); color: var(--primary);">
            BẢN ĐỒ CÂU HỎI
          </h3>
          ...
          <div class="legend-list">
            {#if appState === 'practicing'}
              <div class="legend-item">
                ...
                <span class="legend-text">Đã trả lời</span>
              </div>
              <div class="legend-item">
                <span class="legend-color" style="border: 1px solid var(--border-color); background-color: var(--card-bg);"></span>
                <span class="legend-text">Chưa trả lời</span>
              </div>
              <div class="legend-item">
                <span class="legend-color" style="border: 2px solid var(--primary); background-color: var(--card-bg);"></span>
...
<footer>
  <div style="text-align: center; padding: 2rem 1rem; font-size: 0.85rem; color: var(--text-muted); border-top: 1px solid var(--border-color); background-color: var(--card-bg); margin-top: 3rem;">
    © 2026 Lực lượng tham gia bảo vệ an ninh, trật tự ở cơ sở. Phát triển cho kỳ thi trắc nghiệm học tập.
  </div>
</footer>
